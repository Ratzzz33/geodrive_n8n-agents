...
async function fixTriggerParseJsonbString() {
  const sql = postgres(CONNECTION_STRING, {
    max: 1,
    ssl: { rejectUnauthorized: false },
  });

  console.log('\n🔧 Исправление триггера: парсинг JSONB-строки...\n');
  ...
  try {
    ...
    await sql.unsafe(`
      ...
      BEGIN
        ...
        -- КРИТИЧНО: Если NEW.data - это JSONB-строка, распарсить в объект
    ...
    console.log('✅ Триггер исправлен!');
    console.log('📝 Теперь триггер:');
    console.log('   1. Распарсивает JSONB-строку в объект');
    console.log('   2. Раскладывает поля брони (start_date, end_date, state, price, days, total, deposit)');
    console.log('   3. Обрабатывает вложенные car и client');
    console.log('   4. Создает/обновляет записи в cars и clients');
    console.log('   5. Устанавливает car_id и client_id в booking');
