...
try {
  console.log('📝 Исправляем обработку UUID foreign keys...');
  ...
  await sql`
    ...
    BEGIN
      ...
      LOOP
        ...
        -- Пропускаем UUID foreign key колонки (заканчиваются на _id)
        -- Они должны устанавливаться через mapping, не из сырых RentProg данных
  ...
  console.log('   - Пропускаются системные поля: id, rentprog_id, created_at, updated_at');
  console.log('   - Пропускаются UUID foreign keys: *_id (кроме rentprog_id)');
  console.log('   - Foreign keys будут устанавливаться через отдельный mapping');
