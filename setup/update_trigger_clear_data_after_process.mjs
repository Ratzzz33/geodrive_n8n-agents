...
async function updateTriggerClearData() {
  const sql = postgres(CONNECTION_STRING, {
    max: 1,
    ssl: { rejectUnauthorized: false },
  });

  console.log('\n🔧 Обновление триггера: очистка data после обработки...\n');
  ...
  try {
    ...
    await sql.unsafe(`
      ...
      DECLARE
        ...
        original_data JSONB;
      BEGIN
        -- Если data пустой или уже обработан - пропускаем
        IF NEW.data IS NULL OR NEW.data::TEXT = '{}' THEN
          RETURN NEW;
        END IF;
        ...
        -- КРИТИЧНО: Если data - это JSONB-строка, распарсить в объект
        ...
        -- Устанавливаем data = {} для визуального контроля обработанных записей
        NEW.data := '{}'::JSONB;
    ...
    console.log('📝 Теперь триггер:');
    console.log('   1. Пропускает записи с пустым data (уже обработанные)');
    console.log('   2. Парсит JSONB-строку в объект (если нужно)');
    console.log('   3. Раскладывает поля брони');
    console.log('   4. Обрабатывает вложенные car и client');
    console.log('   5. ОЧИЩАЕТ data = {} после обработки');
    console.log('   6. Позволяет визуально видеть обработанные записи');
