...
async function fixSaveCarsNode() {
  ...
  console.log('🔧 Исправляю ноду "Save Cars" (замена на dynamic_upsert_entity)...');
  ...
  // Заменяем SQL запрос на вызов dynamic_upsert_entity
  ...
  console.log('✅ Нода "Save Cars" обновлена (использует dynamic_upsert_entity)\n');
  ...
  console.log('  ✅ Нода "Save Cars" теперь использует dynamic_upsert_entity');
  console.log('  ✅ Это решает проблему с ON CONFLICT (rentprog_id)');
  console.log('  ✅ Функция правильно обрабатывает upsert и защищает от NULL значений');
  console.log('\n✅ Готово!');
}
