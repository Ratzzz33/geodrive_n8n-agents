...
// Полный workflow из версии 256 (snapshot с правильными Check Exists и If Exists)
const workflowData = {
  ...
  "connections": {
    ...
    "Parse Webhook": {
      "main": [[{"node": "Save to Events", "type": "main", "index": 0}]]
    },
    "Save to Events": {
      "main": [[{"node": "Pass Data", "type": "main", "index": 0}]]
    },
    "Pass Data": {
      "main": [[{"node": "Switch by Operation", "type": "main", "index": 0}]]
    },
    "Switch by Operation": {
      "main": [
    ...
    "Prepare Create": {
      "main": [[{"node": "Insert Entity", "type": "main", "index": 0}]]
    },
    "Insert Entity": {
    ...
    "If Exists": {
      "main": [
        ...
        [{"node": "Get RentProg Token", "type": "main", "index": 0}]
      ]
    },
    "Prepare Update": {
      "main": [[{"node": "Update Entity", "type": "main", "index": 0}]]
    },
    "Update Entity": {
      "main": [[{"node": "Respond Success", "type": "main", "index": 0}]]
    },
    "Get RentProg Token": {
      "main": [[{"node": "Switch by Entity", "type": "main", "index": 0}]]
    },
    "Switch by Entity": {
      "main": [
        [{"node": "Fetch Car", "type": "main", "index": 0}],
        [{"node": "Fetch Client", "type": "main", "index": 0}],
        [{"node": "Fetch Booking", "type": "main", "index": 0}]
      ]
    },
    "Fetch Car": {
...
console.log('\n🔄 Восстановление Service Center Processor (версия 256, 20 нод)...\n');
...
const req = https.request(`https://n8n.rentflow.rentals/api/v1/workflows/${WORKFLOW_ID}`, options, (res) => {
  ...
  res.on('end', () => {
    ...
    if (res.statusCode === 200) {
      ...
      console.log('   ✓ 20 нод (полная структура)');
      console.log('   ✓ Check Exists с COALESCE и EXISTS');
      console.log('   ✓ If Exists проверяет boolean exists');
      console.log('   ✓ Все connections восстановлены');
      console.log('   ✓ Settings: saveDataSuccessExecution: all\n');
    } else {
