...
async function fixDynamicUpsertFunction() {
  ...
  try {
    ...
    console.log('   - Исключены служебные поля и foreign keys из SET clause:');
    console.log('     • id (primary key)');
    console.log('     • created_at (не должно обновляться)');
    console.log('     • updated_at (обновляется автоматически)');
    console.log('     • car_id, client_id, booking_id (foreign keys - UUID, конфликт с integer из RentProg)');
    console.log('   - updated_at добавляется отдельно в конце UPDATE');
    console.log('   - external_id заполняется при INSERT в external_refs\n');

    console.log('💡 Теперь не будет конфликта при обновлении записей\n');
