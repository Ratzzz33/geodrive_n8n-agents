...
try {
  console.log('📝 Исправляем дубликат rentprog_id...');
  ...
  await sql`
    ...
    BEGIN
      ...
      LOOP
        -- Пропускаем rentprog_id, так как он передаётся отдельным параметром
        IF key = 'rentprog_id' THEN
          CONTINUE;
        END IF;
      ...
      ELSE
        -- UPDATE (также обновляем rentprog_id на случай если он изменился)
        sql_query := format(
  ...
  console.log('   - rentprog_id пропускается при обработке JSON (CONTINUE)');
  console.log('   - Устранён дубликат колонки');
