 ...
 * Используем полное обновление workflow через n8n API
 */
...
try {
  ...
  // Обновляем код ноды - добавляем client_id
  const updatedCode = processNode.parameters.jsCode
    .replace(
  ...
  processNode.parameters.jsCode = updatedCode;
