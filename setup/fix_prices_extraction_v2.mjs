...
async function fixPricesExtractionV2() {
  ...
  // Исправляем ноду "Merge & Process" - улучшаем обработку included
  console.log('🔧 Исправляю ноду "Merge & Process" (улучшаю обработку included)...');
  ...
  // Обновляем код - добавляем проверку на массив и более детальную обработку
  const currentCode = mergeProcessNode.parameters.jsCode;
  
  // Заменяем блок извлечения included на более надежный
  const updatedCode = currentCode.replace(
    ...
    `  // Извлекаем included (цены и сезоны) - пробуем разные варианты структуры\n  let included = [];\n  if (responseData.included && Array.isArray(responseData.included)) {\n    included = responseData.included;\n  } else if (responseData.cars && responseData.cars.included) {\n    // Проверяем, массив ли это\n    if (Array.isArray(responseData.cars.included)) {\n      included = responseData.cars.included;\n    } else if (typeof responseData.cars.included === 'object') {\n      // Если это объект, пробуем преобразовать в массив\n      included = Object.values(responseData.cars.included);\n    }\n  } else if (responseData.data && responseData.data.included && Array.isArray(responseData.data.included)) {\n    included = responseData.data.included;\n  }`
  );
  
  mergeProcessNode.parameters.jsCode = updatedCode;
  console.log('✅ Нода "Merge & Process" обновлена с улучшенной обработкой included\n');
  ...
  console.log('  ✅ Добавлена проверка на массив для included');
  console.log('  ✅ Добавлена обработка случая, когда included - объект');
  console.log('  ✅ Улучшена надежность извлечения цен');
  console.log('\n⚠️  ВАЖНО: Протестируйте workflow, чтобы убедиться, что цены извлекаются!');
}

fixPricesExtractionV2()
  .then(() => {
    console.log('\n✅ Готово!');
    process.exit(0);
  })
