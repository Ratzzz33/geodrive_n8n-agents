-- Migration: Fix field change parsing for decimal numbers (e.g., 7.4)
-- Date: 2025-11-20
-- Author: Cursor Agent
-- Issue: Parser stops at dot in decimal numbers (7.4 becomes 7)

-- Update parse_history_description to handle decimal numbers correctly
...
BEGIN
  ...
  -- FIXED: Handle decimal numbers: "7.4" should be captured fully, not just "7"
  ...
  IF operation = 'update' AND description_text ~* 'изменил\s*[,]?\s*(\w+)\s+с\s+(.+?)\s+на\s+' THEN
    ...
    -- Extract new value - FIXED: capture decimal numbers correctly
    -- Stop at " в " (with spaces), comma, period (if not part of number), or end
    -- Use [^\s]+? to capture everything including dots until space before " в "
    new_value := substring(description_text from 'на\s+([^\s]+?)(?:\s+в\s+|$)');
    -- If that didn't work, try with comma/period as stop (but allow dots in numbers)
    IF new_value IS NULL OR new_value = '' THEN
      -- Try to capture number with optional decimal part
      new_value := substring(description_text from 'на\s+(\d+(?:\.\d+)?|[^,\.\s]+)(?:\s*[,\.]|\s+в\s+|$)');
    END IF;
    -- Last resort: capture until comma or period (but this might cut decimals)
    IF new_value IS NULL OR new_value = '' THEN
...
BEGIN
  ...
  RAISE NOTICE 'Now correctly handles decimal numbers in field changes (e.g., 7.4 instead of 7)';
END $$;
