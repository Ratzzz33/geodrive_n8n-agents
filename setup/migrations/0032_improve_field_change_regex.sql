-- Migration: Improve field change parsing regex
-- Date: 2025-11-20
-- Author: Cursor Agent
-- Issue: Parser captures too much text in new_value

-- Update parse_history_description with improved regex
...
BEGIN
  ...
  -- Improved regex: stop at comma, period, or " в "
  IF operation = 'update' AND description_text ~* 'изменил\s+(\w+)\s+с\s+(.+?)\s+на\s+([^,\.]+?)(?:\s*[,\.]|\s+в|\s*$)' THEN
    -- Extract field name
    ...
    -- Extract new value (stop at comma, period, or " в ")
...
BEGIN
  ...
  RAISE NOTICE 'Successfully updated parse_history_description with improved regex';
  RAISE NOTICE 'Now correctly extracts only the new value without trailing text';
END $$;
