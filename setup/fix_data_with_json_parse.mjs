...
async function main() {
  try {
    ...
    console.log('🔧 Возвращаемся к upsert с JSON.parse() для data...\n');
    
    // Возвращаем upsert с правильным маппингом data через JSON.parse
    saveNode.parameters = {
      operation: 'upsert',
      schema: {
        '__rl': true,
      ...
      columns: {
        ...
        'value': {
          ...
          'data': '={{ JSON.parse($json.payload_json) }}',  // ✅ JSON.parse преобразует строку → объект
    ...
    console.log('   data: JSON.parse($json.payload_json) - преобразует строку → объект');
    ...
    console.log('');
    console.log('🎯 Теперь data = JSON.parse(payload_json)');
    console.log('   Postgres нода получит ОБЪЕКТ и сериализует его в JSONB');
    console.log('');
