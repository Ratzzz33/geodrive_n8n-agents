...
async function showStarlineData() {
  try {
    console.log('🔍 Данные Starline для Mercedes Benz GLE 350 (OB700OB)\n');
    ...
    const car = cars[0];
    console.log(`\n📋 Машина: ${car.car_visual_name || car.model || 'N/A'} (${car.plate})`);
    console.log(`   Филиал: ${car.branch_name || 'N/A'} (${car.branch_code || 'N/A'})`);
    console.log(`   Car ID: ${car.id}\n`);

    // 1. Данные из starline_devices
    console.log('═'.repeat(80));
    console.log('📡 ДАННЫЕ STARLINE ИЗ БД:\n');
    ...
    if (starlineDevices.length === 0) {
      console.log('❌ Устройства Starline не найдены для этой машины\n');
    } else {
      starlineDevices.forEach((device, index) => {
        console.log(`\n${index + 1}. Устройство Starline:`);
    ...
    // 3. История координат из entity_timeline (GPS события от Starline)
    console.log('\n' + '═'.repeat(80));
