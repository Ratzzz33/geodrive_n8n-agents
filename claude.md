...
### n8n через MCP ⭐ (ОСНОВНОЙ СПОСОБ для агента)

**ВАЖНО:** Для работы с n8n используй **ТОЛЬКО MCP инструменты**. REST API оставлен для справки.
...
**Как использовать:**
- ✅ **Для всех операций с n8n:** `mcp_n8n_n8n_...` (быстрее, проще)
  - `mcp_n8n_n8n_list_workflows()` - список workflow
  - `mcp_n8n_n8n_get_workflow(id)` - получить workflow
  - `mcp_n8n_n8n_execute_workflow(id)` - запустить workflow
  - `mcp_n8n_n8n_create_workflow(...)` - создать workflow
- ⚙️ **Для документации узлов:** `mcp_n8n-mcp-official_...`
  - `mcp_n8n-mcp-official_get_node_info(nodeType)` - документация узла
  - `mcp_n8n-mcp-official_search_nodes(query)` - поиск узлов
- 🔄 **Резервный вариант:** если основной не работает → переключись на `mcp_n8n-mcp-official_n8n_...`
...
## 🎨 Best Practices: Проектирование надежных n8n workflow

**Важно:** См. также [.cursorrules](./.cursorrules) - раздел "BEST PRACTICES: Проектирование надежных workflow" для полных правил.

### 1. Retry механизм для HTTP Request нод

**ВСЕГДА добавляй для внешних API запросов:**
- `retryOnFail: true`
- `maxTries: 2`
- `continueOnFail: true`

**Польза:** Автоматический повтор при временных сбоях сети/API, повышение надежности без ручного вмешательства.

**Когда использовать:**
- ✅ HTTP Request к внешним API (RentProg, AmoCRM, Umnico)
- ✅ Postgres запросы (опционально, если критично)
- ❌ НЕ использовать для внутренних нод (Code, IF, etc.)

### 2. Проверка ошибок в IF нодах
...
**Почему:**
- `error_count > 0` более точная проверка, чем `success === false`
- Прямая проверка количества ошибок, а не булевого флага
- Соответствие требованиям n8n API 2025 (обязательный `id`)

### 3. HTML ссылки на execution в Telegram уведомлениях
...
**Польза:** Быстрый переход к execution из Telegram, удобство отладки без поиска в UI.

### 4. Понятные сообщения в уведомлениях

**❌ НЕПРАВИЛЬНО:**
```javascript
message = '📜 HISTORY PARSER\n';
message = '💰 COMPANY CASH\n';
```

**✅ ПРАВИЛЬНО:**
```javascript
message = '📜 Парсинг истории операций по филиалам раз в 3 минуты:\n';
message = '💰 Парсинг касс компании раз в 3 минуты\n';
```

**Польза:** Легче понять источник уведомления, понятнее контекст при множестве workflow.

### 5. Структура workflow для обработки ошибок

**Рекомендуемая структура:**
```
Format Result → If Error → [Send Alert → Throw Error] / [Success]
```

**Правила:**
- ✅ True branch (ошибка) → Send Alert → Throw Error
- ✅ False branch (успех) → Success (noOp нода)
- ✅ Уведомления ТОЛЬКО при ошибках
- ✅ Execution помечается как failed при ошибках

### 6. Добавление ноды Success

**ВСЕГДА добавляй ноду Success для визуализации успешного завершения:**
```json
{
  "name": "Success",
...
**Подключение:** False branch ноды "If Error" → "Success"

**Польза:** Визуальная индикация успешного выполнения, улучшение читаемости workflow.

**Полные правила:** [.cursorrules](./.cursorrules) - раздел "BEST PRACTICES: Проектирование надежных workflow"
...
**Примечание:** Для работы агента используй MCP инструменты (см. секцию выше). REST API оставлен для справки и ручных операций.
...
- **[AGENTS.md](./AGENTS.md)** - Список всех агентов
