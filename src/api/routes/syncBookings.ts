  ...
  try {
    ...
    for (const branch of branches) {
      ...
      try {
        ...
        // ВАЖНО: API /all_bookings игнорирует per_page и всегда возвращает 10 записей на страницу
        // Поэтому пагинация продолжается, пока получаем 10 записей, останавливается при < 10
        ...
        const bookings = await paginate<any>(branch, '/all_bookings', {
          per_page: 10, // API игнорирует это значение, но указываем для ясности
        });
