# Документация переменных окружения n8n

## Обзор

Этот документ содержит полный список всех переменных окружения, используемых в n8n проекте.

---

## Типы переменных

### 🔧 Системные переменные
- Управляются через `docker-compose.yml`
- Требуют перезапуск контейнера для применения изменений
- Нельзя изменить через n8n API

### 👤 Пользовательские переменные
- Доступны в workflow как `{{ $env.VARIABLE_NAME }}`
- Можно изменять через n8n Settings API (в некоторых случаях)
- Также управляются через `docker-compose.yml`

---

## Таблица переменных

| Имя | Тип | Требует перезапуск | Текущее значение | Описание |
|-----|-----|-------------------|------------------|----------|
| **Webhook URLs** |||||
| `WEBHOOK_URL` | Системная | ✅ Да | `https://webhook.rentflow.rentals` | Базовый URL для production webhook |
| `N8N_WEBHOOK_URL` | Системная | ✅ Да | `https://webhook.rentflow.rentals` | Альтернативный webhook URL |
| `WEBHOOK_TEST_URL` | Системная | ✅ Да | `https://webhook-test.rentflow.rentals` | URL для тестовых webhook |
| **n8n Host настройки** |||||
| `N8N_HOST` | Системная | ✅ Да | `n8n.rentflow.rentals` | Домен n8n |
| `N8N_PORT` | Системная | ✅ Да | `5678` | Порт n8n |
| `N8N_PROTOCOL` | Системная | ✅ Да | `https` | Протокол (http/https) |
| `N8N_EDITOR_BASE_URL` | Системная | ✅ Да | `https://n8n.rentflow.rentals` | Базовый URL редактора |
| **Безопасность** |||||
| `N8N_ENCRYPTION_KEY` | Системная | ✅ Да | (автоматически) | Ключ шифрования данных |
| **База данных** |||||
| `POSTGRES_HOST` | Системная | ✅ Да | `ep-rough-heart-ahnybmq0-pooler.c-3.us-east-1.aws.neon.tech` | Хост PostgreSQL (Neon) |
| `POSTGRES_PORT` | Системная | ✅ Да | `5432` | Порт PostgreSQL |
| `POSTGRES_DB` | Системная | ✅ Да | `neondb` | Имя базы данных |
| `POSTGRES_USER` | Системная | ✅ Да | `neondb_owner` | Пользователь БД |
| `POSTGRES_PASSWORD` | Системная | ✅ Да | `npg_cHIT9Kxfk1Am` | Пароль БД |
| **API Endpoints** |||||
| `RENTPROG_HEALTH_URL` | Пользовательская | ⚠️ Рекомендуется | `http://46.224.17.15:3000/rentprog/health` | URL для health check RentProg |
| `ORCHESTRATOR_URL` | Пользовательская | ⚠️ Рекомендуется | `http://46.224.17.15:3000` | URL оркестратора |
| `API_BASE_URL` | Пользовательская | ⚠️ Рекомендуется | `https://api.rentflow.rentals` | Базовый URL API |
| `SYNC_STATUS_URL` | Пользовательская | ⚠️ Рекомендуется | `http://46.224.17.15:3000/sync/status` | URL для проверки статуса синхронизации |
| **Интеграции** |||||
| `TELEGRAM_ALERT_CHAT_ID` | Пользовательская | ❌ Нет | `-5004140602` | ID чата для алертов в Telegram |

---

## Инструкции по обновлению

### Для системных переменных

1. **Обновить конфигурацию:**
   ```bash
   python setup/manage_n8n_variables.py update WEBHOOK_URL=https://new-url.com
   ```

2. **Синхронизировать с сервером:**
   ```bash
   python setup/manage_n8n_variables.py sync
   ```

   Или вручную:
   ```bash
   # На сервере
   cd /root/geodrive_n8n-agents
   # Отредактировать docker-compose.yml
   docker compose stop n8n
   docker compose up -d n8n
   ```

3. **Проверить:**
   ```bash
   python setup/server_ssh.py "docker exec n8n printenv WEBHOOK_URL"
   ```

### Для пользовательских переменных

**Способ 1: Через manager (рекомендуется)**
```bash
python setup/manage_n8n_variables.py update TELEGRAM_ALERT_CHAT_ID=-123456789
python setup/manage_n8n_variables.py sync
```

**Способ 2: Через n8n Settings API** (если доступен)
```python
# В разработке
```

**Способ 3: Вручную в docker-compose.yml**
1. Отредактировать `docker-compose.yml`
2. Перезапустить контейнер (рекомендуется)

---

## Проверка переменных

### Локально
```bash
# Список всех переменных
python setup/manage_n8n_variables.py list

# Только системные
python setup/manage_n8n_variables.py list --type system

# Только пользовательские
python setup/manage_n8n_variables.py list --type user
```

### На сервере
```bash
# Все переменные
python setup/server_ssh.py "docker exec n8n printenv"

# Только WEBHOOK переменные
python setup/server_ssh.py "docker exec n8n printenv | grep WEBHOOK"
```

### Проверка синхронизации
```bash
# Локальная проверка
python setup/validate_env_sync.py

# Строгая проверка (предупреждения = ошибки)
python setup/validate_env_sync.py --strict

# Отчет
python setup/validate_env_sync.py --report
```

---

## Часто задаваемые вопросы

### Почему переменная не обновилась после изменения в docker-compose.yml?

Системные переменные требуют перезапуска контейнера:
```bash
docker compose stop n8n && docker compose up -d n8n
```

`docker compose restart n8n` не перечитывает переменные окружения!

### Как проверить, что переменная применилась?

```bash
python setup/server_ssh.py "docker exec n8n printenv VARIABLE_NAME"
```

### Можно ли изменить переменную через n8n API?

Системные переменные - **НЕТ**, только через docker-compose.yml и перезапуск.

Пользовательские переменные - **ЧАСТИЧНО**, в некоторых версиях n8n через Settings API.

### Где хранится актуальная конфигурация?

Единственный источник правды: `config/n8n-variables.yaml`

Используйте инструменты для синхронизации:
- `python setup/sync_variables.py` - синхронизация с docker-compose.yml
- `python setup/manage_n8n_variables.py sync` - синхронизация с сервером

---

## Инструменты

| Скрипт | Назначение |
|--------|-----------|
| `setup/manage_n8n_variables.py` | Универсальный менеджер переменных |
| `setup/sync_variables.py` | Синхронизация YAML → docker-compose.yml |
| `setup/validate_env_sync.py` | Проверка соответствия |
| `setup/check_env_sync.py` | Обнаружение несоответствий (расширенная версия) |
| `setup/server_ssh.py` | SSH доступ к серверу |
| `setup/update_webhook_url.py` | Автоматическое обновление WEBHOOK_URL |

---

## Архитектура

```
config/n8n-variables.yaml  (Источник правды)
           ↓
    setup/sync_variables.py
           ↓
      docker-compose.yml
           ↓
    setup/manage_n8n_variables.py sync
           ↓
    Сервер: docker-compose.yml
           ↓
    docker compose restart n8n
           ↓
    Контейнер n8n (переменные применены)
```

---

## Безопасность

⚠️ **Важно:** Не коммитьте пароли и ключи в Git!

Используйте `.env` файл для хранения чувствительных данных:
```bash
# .env (не коммитится)
POSTGRES_PASSWORD=your_secure_password
N8N_ENCRYPTION_KEY=your_encryption_key
```

Затем в docker-compose.yml:
```yaml
environment:
  - POSTGRES_PASSWORD=${POSTGRES_PASSWORD}
```

---

## См. также

- [SSH_CONNECTION_GUIDE.md](../setup/SSH_CONNECTION_GUIDE.md) - Подключение к серверу
- [claude.md](../claude.md) - Рабочие решения для агента
- [README.md](../README.md) - Основная документация

---

**Последнее обновление:** 2025-11-03  
**Версия:** 1.0.1

