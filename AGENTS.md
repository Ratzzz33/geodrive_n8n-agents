...
- ✅ **Улучшения архитектуры агентов:** Рекомендации по улучшению в `docs/AGENT_PATTERNS_IMPROVEMENTS.md` (Agent Execution Logging, Agent Memory/Context, Agent Tool Registry, Agent Retry Strategy, ReAct Pattern, Agent Evaluation) — см. roadmap в `NEXT_STEPS.md`
...
## 35. Ночной агент продаж (AmoCRM‑only MVP)

**Триггеры**: Cron (ночное окно), `amocrm.conversation.pending`

**Функции**:
- Поиск диалогов, ожидающих ответа, через AmoCRM “Чаты в CRM”
- Формирование ответа на основе RAG (победные диалоги) и шаблонов
- Отправка ответа в Amo, ведение логов/метрик, эскалация риск‑кейсов

**Действия**:
- Автоответы по белому списку интентов
- Создание задач/эскалаций при сомнениях
- Утренние сводки (KPI: ответ клиента, переходы по стадиям, конверсия)

**Интеграции**: AmoCRM API v4 (“Сделки”, “Чаты в CRM”), Neon (pgvector), n8n AI Agent/Ollama

**Ограничения**:
- Запрет изменения цен/депозитов/условий
- Лимиты частоты и количества ответов
- PII‑политики при эмбеддинге и хранении сырых данных
