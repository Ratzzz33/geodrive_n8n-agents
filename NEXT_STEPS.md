...
- Подсистема задач (этап 1):
  - Создать миграции для `tasks`, `task_links`, `task_events`, `employees.task_chat_id`
...
- Подсистема задач (этап 2):
  ...
  - Связки задач с `car/booking/client/branch` через `task_links`
  - Списки в Jarvis: личные/по филиалу/по сущности; быстрый фильтр по статусу/приоритету
...
- Подсистема задач (этап 3):
  - Расширенные чек-листы, шаблоны задач
  - Интеграция с дневным планом; автогенерация подзадач
  - Персональные KPI по закрытию задач; отчеты по филиалам
