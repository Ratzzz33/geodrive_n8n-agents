...
- **MCP Server** - официальный сервер `@makafeli/n8n-workflow-builder` для интеграции с Cursor
- **Neon PostgreSQL** - облачная база данных
- **Docker & Docker Compose** - для контейнеризации сервисов

## 🚀 Быстрая настройка MCP для Cursor

Для использования n8n через Cursor с поддержкой создания агентов и оркестратора:

1. **Установка MCP сервера (уже выполнено)**:
   ```bash
...
3. **Настройка Cursor**:
   - См. подробную инструкцию: [MCP_SETUP.md](MCP_SETUP.md)

**Документация:**
- 📘 [MCP_SETUP.md](MCP_SETUP.md) - Настройка MCP для Cursor
- 🤖 [N8N_AGENTS_SETUP.md](N8N_AGENTS_SETUP.md) - Настройка агентов в n8n
